...
# MAGIC %sql
# MAGIC SELECT count(*) as sum_of_rows FROM events
...
# MAGIC %sql
# MAGIC CREATE DATABASE training;

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS training.events USING parquet OPTIONS (path "/mnt/training/ecommerce/events/events.parquet");

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS training.users USING parquet OPTIONS (path "/mnt/training/ecommerce/users/users.parquet");
# MAGIC CREATE TABLE IF NOT EXISTS training.sales USING parquet OPTIONS (path "/mnt/training/ecommerce/sales/sales.parquet");
# MAGIC CREATE TABLE IF NOT EXISTS training.products USING parquet OPTIONS (path "/mnt/training/ecommerce/products/products.parquet");
...
# MAGIC %sql
# MAGIC select * from training.products
...
# MAGIC %sql
# MAGIC SELECT ROUND(SUM(purchase_revenue_in_usd)/COUNT(purchase_revenue_in_usd), 2) as Average from training.sales
...
# MAGIC %sql
# MAGIC select distinct event_name from training.events
