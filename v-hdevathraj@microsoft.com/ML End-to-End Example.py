...
white_wine = pd.read_csv("/dbfs/FileStore/shared_uploads/v-hdevathraj@microsoft.com/winequality_white.csv", sep=';')
red_wine = pd.read_csv("/dbfs/FileStore/shared_uploads/v-hdevathraj@microsoft.com/winequality_red.csv", sep=';')
...
# MAGIC ## Model serving
# MAGIC 
# MAGIC To productionize the model for low latency predictions, use MLflow Model Serving ([AWS](https://docs.databricks.com/applications/mlflow/model-serving.html)|[Azure](https://docs.microsoft.com/azure/databricks/applications/mlflow/model-serving)) to deploy the model to an endpoint.
# MAGIC 
